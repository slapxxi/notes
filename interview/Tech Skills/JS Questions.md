...
> The value of `this` depends on how a function is called - in the global context it refers to `window` (or `undefined` in strict mode), in object methods it refers to that object, and in event listeners it usually refers to the element. Arrow functions don't bind their own `this`; they inherit it _lexically_.
